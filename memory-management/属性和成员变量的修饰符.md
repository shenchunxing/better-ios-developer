...
### __weak修饰基本数据类型，会有什么问题？
基本数据类型（如 int、float、bool 等）不是 Objective-C 对象，它们是直接存储在栈上的简单数据类型。因此，__weak 修饰符不能用于基本数据类型，也没有对应的内存管理机制。强制使用 __weak 修饰基本数据类型实际上是没有效果的，因为基本数据类型不是 Objective-C 对象，无法享受 __weak 修饰符提供的弱引用机制。
...
其中， block 的 property 设置为 copy， 对应的是这一条：将 block 赋值给 __strong 指针时。
  ...
  这里还是建议写_userId = [userId copy];而不是直接_userId = userId；因为防止传入的userId可能是一个可变的，导致_userId也变化了。
 ```Objective-C
...
> runtime 对注册的类，会进行布局，对于 weak 对象会放入一个 hash 表中。 用 weak 指针(obj1、objA)指向的对象(obj0)内存地址作为 key，当此对象的引用计数为0的时候会反向找到 weak 指针(obj1、objA) 并 dealloc。假如 weak 指针(obj1、objA)指向的对象(obj0)内存地址是a，那么就会以a为键， 在这个 weak 表中搜索，找到所有以a为键的 weak指针(obj1、objA)，从而将weak指针设置为 nil。
