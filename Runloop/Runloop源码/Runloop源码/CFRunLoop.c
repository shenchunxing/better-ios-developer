...
//MARK: - __CFRunLoopMode底层结构，表示运行模式
struct __CFRunLoopMode {
    ...
    Boolean _stopped;
    char _padding[3];
    CFMutableSetRef _sources0;//MARK: -  触摸事件、performselector方法
    CFMutableSetRef _sources1; //MARK: - 基于port的线程间通信、系统事件捕捉
    CFMutableArrayRef _observers;//MARK: - 用于监听runloop状态、ui刷新(休眠之前)、autorelease pool(休眠之前)
    CFMutableArrayRef _timers;//MARK: - nstimer、performselector：withobject:afterdelay
...
struct __CFRunLoop {
    ...
    CFMutableSetRef _commonModeItems;//当前运行在common mode模式下的CFRunLoopSource，CFRunLoopObserver，CFRunLoopTimer
    CFRunLoopModeRef _currentMode; //只有其中一个CFMutableSetRef是_currentMode
    CFMutableSetRef _modes; //runloop有多个CFMutableSetRef
...
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    ...
    //MARK: - 根据key获取value，key是线程
    CFRunLoopRef loop = (CFRunLoopRef)CFDictionaryGetValue(__CFRunLoops, pthreadPointer(t));
    __CFUnlock(&loopsLock);
    //MARK: - 如果runloop不存在，就创建
    if (!loop) {
	CFRunLoopRef newLoop = __CFRunLoopCreate(t);
        __CFLock(&loopsLock);
	loop = (CFRunLoopRef)CFDictionaryGetValue(__CFRunLoops, pthreadPointer(t));
	if (!loop) {
        //MARK: - 创建完成，保存到全局字典__CFRunLoops中
...
#pragma mark - 通知observers，即将处理timers
        if (rlm->_observerMask & kCFRunLoopBeforeTimers) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeTimers);
#pragma mark - 通知observers，即将处理sources
        if (rlm->_observerMask & kCFRunLoopBeforeSources) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeSources);
#pragma mark - 处理blocks
	__CFRunLoopDoBlocks(rl, rlm);
#pragma mark - 处理source0
        Boolean sourceHandledThisLoop = __CFRunLoopDoSources0(rl, rlm, stopAfterHandle);
#pragma mark - 继续处理blocks
        if (sourceHandledThisLoop) {
            __CFRunLoopDoBlocks(rl, rlm);
	}

        Boolean poll = sourceHandledThisLoop || (0ULL == timeout_context->termTSR);
#pragma mark - 判断是否有source1
        if (MACH_PORT_NULL != dispatchPort && !didDispatchPortLastTime) {
#if DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED || DEPLOYMENT_TARGET_EMBEDDED_MINI
            msg = (mach_msg_header_t *)msg_buffer;
            if (__CFRunLoopServiceMachPort(dispatchPort, &msg, sizeof(msg_buffer), &livePort, 0, &voucherState, NULL)) {
#pragma mark - 如果有source1，跳转到handle_msg
                goto handle_msg;
            }
#elif DEPLOYMENT_TARGET_WINDOWS
...
#pragma mark - 通知observer，结束休眠
	__CFRunLoopUnsetSleeping(rl);
	if (!poll && (rlm->_observerMask & kCFRunLoopAfterWaiting)) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopAfterWaiting);

#pragma mark - 判断是怎么醒来的，做相应的事情
        handle_msg:;
        __CFRunLoopSetIgnoreWakeUps(rl);
...
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds, Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
    ...
    #pragma mark - 通知observer，进入runloop
	if (currentMode->_observerMask & kCFRunLoopEntry ) __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopEntry);
    #pragma mark - 具体要做的事情，主逻辑
	result = __CFRunLoopRun(rl, currentMode, seconds, returnAfterSourceHandled, previousMode);
    #pragma mark - 通知observer，退出runloop
