...
// MARK: - 根据key和mask查找存储在哈希表中的下标。直接&运算
...
bucket_t * cache_t::find(cache_key_t k, id receiver)
{
    ...
    bucket_t *b = buckets();//方法列表
    mask_t m = mask(); //掩码
    mask_t begin = cache_hash(k, m);//根据key和mask查找存储在哈希表中的下标。直接&运算
    mask_t i = begin;//begin就是i，i就是索引
    do {
        //如果取出来的key和传入的key是一样的，说明就是我要找的，直接返回找到的cache_t
        if (b[i].key() == 0  ||  b[i].key() == k) {
            return &b[i];
        }
        //如果不是，则继续查找，如果是arm64，则去i - 1的位置查找
